		...
		for k,v in pairs(Config.Menus) do
			...
			if k == data.current.modType or isRimMod then
				...
				if data.current.label == _U('by_default') or string.match(data.current.label, _U('installed')) then
					...
					local vehiclePrice = Config.DefaultPrice
...
function GetXenonColor(angka)
	local warna = {"White", "Blue", "Electric Blue", "Mint Green", "Lime Green", "Yellow", "Golden Shower", "Orange", "Red", "Pony Pink", "Hot Pink", "Purple", "Blacklight", "Default Xenon"}
	return warna[angka]
end

function IsAuthorized( ... )
	return PlayerData.job and PlayerData.job.name == 'bennys'
end
...
function UpdateMods(data)
	...
	if data.modType then
		...
		if data.wheelType then
			...
			props = {}
		elseif data.modType == 'modExtra' then
			props['extras'] = {}
			for id,enabled in pairs(myCar.extras) do
				if tonumber(data.modNum) == tonumber(id) then
					props['extras'][id] = data.modValue
				else
					props['extras'][id] = enabled
				end
			end
			ESX.Game.SetVehicleProperties(vehicle, props)
	...
	for k,v in pairs(Config.Menus) do
		...
		if data.value == k then
			...
			if v.modType then
				...
				if v.modType == 18 then -- disable turbo
					table.insert(elements, {label = _U('by_default'), modType = k, modNum = 0})
				elseif v.modType == 22 then -- disable neon
					table.insert(elements, {label = _U('by_default'), modType = k, modNum = 0, color = 255})
				...
				if v.modType == 14 then -- HORNS
					for j = 0, 51, 1 do
						...
						if j == currentMods.modHorns then
							_label = GetHornName(j) .. ' : <span style="color:tomato;">'.. _U('installed') ..'</span>'
						else
							price = math.floor(vehiclePrice * v.price / 100)
				...
				elseif v.modType == 'plateIndex' then -- PLATES
					for j = 0, 4, 1 do
						...
						if j == currentMods.plateIndex then
							_label = GetPlatesName(j) .. ' : <span style="color:tomato;">'.. _U('installed') ..'</span>'
						else
							price = math.floor(vehiclePrice * v.price / 100)
				...
				elseif v.modType == 22 then -- NEON
					...
					if currentMods.modXenon then
						_label = _U('neon') .. ' : <span style="color:tomato;">'.. _U('installed') ..'</span>'
					else
						price = math.floor(vehiclePrice * v.price / 100)
				...
				elseif v.modType == 'windowTint' then -- WINDOWS TINT
					for j = 1, 5, 1 do
						local _label = ''
						if j == currentMods.modHorns then
							_label = GetWindowName(j) .. ' : <span style="color:tomato;">'.. _U('installed') ..'</span>'
						else
							price = math.floor(vehiclePrice * v.price / 100)
					...
					for j = 0, modCount, 1 do
						...
						if modName then
							...
							if j == currentMods.modFrontWheels then
								_label = GetLabelText(modName) .. ' : <span style="color:tomato;">'.. _U('installed') ..'</span>'
							else
								price = math.floor(vehiclePrice * v.price / 100)
				...
				elseif v.modType == 11 or v.modType == 12 or v.modType == 13 or v.modType == 15 or v.modType == 16 then
					...
					for j = 0, modCount, 1 do
						...
						if j == currentMods[k] then
							_label = _U('level', j+1) .. ' : <span style="color:tomato;">'.. _U('installed') ..'</span>'
						else
				...
				elseif v.modType == 18 then -- TURBO
					local _label = ''
					if currentMods[k] then
						_label = 'Turbo - <span style="color:tomato;">'.. _U('installed') ..'</span>'
					else
...
Citizen.CreateThread(function()
	while true do
		...
		if IsPedInAnyVehicle(playerPed, false) then
			...
			if IsAuthorized() or not Config.IsMechanicJobOnly then
				for k,v in pairs(Config.Zones) do
